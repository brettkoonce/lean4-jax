...
Chapters 2--10 are \textbf{20 axioms and 52 theorems}, with zero
\texttt{sorry}s. (Earlier drafts shipped 30 axioms; the
foundation flip described above shed 10 of them by promoting the
calculus rules from axioms to theorems and replacing several
opaque forwards with concrete definitions.)

\textbf{This chapter (Ch 2)} does the structural heavy lifting:
1 axiom (\(\pdivMat\) row independence --- the matrix-level
analogue that doesn't fall out of the scalar foundation) and 26
theorems that compose Mathlib's \(\fderiv\) into the VJP toolkit
every later chapter will reuse: the chain rule, additive and
multiplicative fan-in, identity, reindexing, and the matrix-/
3-tensor-level lifts. Plus \(\pdiv\) itself as a definition over
\(\fderiv\).

\textbf{Chapter 3 (MLP)} adds 4 axioms (the guarded ReLU
subgradient, softmax cross-entropy gradient, plus two
composition shortcuts --- \(\texttt{relu\_has\_vjp}\) and
\(\texttt{mlp\_has\_vjp}\) --- that bundle Stage 2 work whose
symbolic proof is mechanical but deferred) and 5 theorems
stitching them together. Dense Jacobians in both arguments are
now proved from the foundation rather than axiomatized.
...
\textbf{Chapter 5 (BN)} adds 2 axioms (BN inverse-stddev
broadcast Jacobian and its smoothness lemma under
\(\varepsilon > 0\)) and 5 theorems. BN affine and centering
Jacobians are now proved.
...
\textbf{Chapter 9 (LN + GELU)} adds 1 axiom (GELU Jacobian) and
2 theorems plus 2 definitions. \(\texttt{geluScalar}\) and its
derivative are now concrete defs; LayerNorm reuses the BN proof
template.

\textbf{Chapter 10 (Attention)} adds 9 axioms and 8 theorems.
Two of the axioms are bottom-level (softmax Jacobian, multi-head
SDPA VJP); the other seven are composition shortcuts where the
full symbolic proof is queued behind the foundation flip. The
theorems still call the matrix-composition kit (chain rule,
fan-in, transpose, rowwise softmax, left/right-constant matmul,
scalar scaling) that Chapter~2 set up; the transformer block
factors through every one of them.
...
\hline
2 (You Are Here)       & 1 + 26 &  1 + 26 & foundation \\
3 (MLP)                & 4 + 5  &  5 + 31 & MLP \\
4 (CNN)                & 2 + 2  &  7 + 33 & $+$ CNN \\
5 (BN)                 & 2 + 5  &  9 + 38 & $+$ BN-nets \\
6 (Residual)           & 0 + 1  &  9 + 39 & $+$ ResNet-34 \\
7 (Depthwise)          & 1 + 2  & 10 + 41 & $+$ MobileNet V2 \\
8 (SE)                 & 0 + 1  & 10 + 42 & $+$ EfficientNet \\
9 (LN + GELU)          & 1 + 2  & 11 + 44 & $+$ transformer infra \\
10 (Attention)         & 9 + 8  & 20 + 52 & $+$ ViT \\
\hline
\end{tabular}
\end{center}

Two observations worth pulling out. First, \textbf{Chapters 6
and 8 add zero axioms}: residual connections and
squeeze-and-excitation are pure composition over primitives
that already exist. That's the scaling claim the book is making
--- new architectures, no new calculus --- in its cleanest form.
Second, \textbf{Chapter 10's spike comes from composition
shortcuts}: seven of the nine new axioms are transformer-level
chains where the symbolic proof is mechanical but not yet
completed (an active axiom-elimination target). The structural
calculus itself never changes from Chapter~2 onward.
...
The Verified VJP Proofs suite contains \textbf{20 axioms, 52
theorems, and 28 definitions} across 9 Lean files. This is a
meaningful shrink from earlier drafts (30 axioms, 45 theorems,
22 definitions): the foundation flip elevated the calculus
axioms (chain rule, sum rule, product rule, identity, reindex)
to theorems proved from Mathlib's \(\fderiv\), and several
...
Jacobians are derived rather than asserted. The 20 surviving
axioms fall into four buckets:

\begin{itemize}
  \item \textbf{Structural residual} (1): \(\pdivMat\) row
    independence --- the matrix-level analogue of \(\pdiv\)'s
    structure that doesn't fall directly out of the scalar
    foundation.
  \item \textbf{Elementary Jacobians} (6): the closed-form
    Jacobian of an operation where the foundation proof is
    deferred --- ReLU's guarded subgradient, softmax
    cross-entropy, BN inverse-stddev broadcast (plus its
    smoothness lemma under \(\varepsilon > 0\)), GELU, and
    softmax. Each has a finite-difference test in
    \texttt{check\_axioms.py}.
  \item \textbf{Bundled input VJPs} (3): \(\texttt{conv2d}\),
    ...
    gradients are now proved from the foundation, so these are
    the only opaque pieces left in their respective layers.
  \item \textbf{Composition shortcuts} (10): higher-level
    chains where the symbolic proof is mechanical but not yet
    completed --- the ReLU and MLP composition shortcuts at the
    Ch~3 boundary, multi-head SDPA, and seven transformer-level
    chains (sublayers with and without residuals, the block,
    the tower, the ViT body). These are valid targets for
    future axiom-elimination work.
\end{itemize}

Everything else --- 52 theorems, 28 definitions --- is
composition over the foundation and these axioms, glued by the
structural rules (chain, additive fan-in, multiplicative fan-in,
three-tensor extension) and the closed-form Jacobian tricks each
chapter introduces. The book's central claim --- ``new
architectures, no new calculus'' --- still holds: each chapter
after Chapter~\ref{chap:tensor} adds at most a handful of
bundled axioms for its new primitive and a small number of
composition theorems to stitch them in. The structural calculus
itself never changes.

\section{Finite-difference gradient checks}

The Elementary-Jacobian and Bundled-input-VJP axioms above are
the ones not proved symbolically in Lean --- they're validated
by finite-difference gradient checks in
