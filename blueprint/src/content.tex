...
and plain SGD. About 7,850 parameters. Trains in seconds. Lands
near 92\% test accuracy on MNIST. The whole training program is
six lines:
...
To compute gradients, we need \(\nabla_W \ell\) and
\(\nabla_b \ell\) where
\(\ell(W, b;\, x, t) = \mathrm{CE}\bigl(\mathrm{softmax}(W x + b),\, t\bigr)\).
Each step below names the theorem (or foreshadows the
Ch~\ref{chap:mlp} theorem) that makes the step formally correct.

\paragraph{Step 1 --- the loss is a composition.}
\(\ell = \mathrm{CE} \circ \mathrm{softmax} \circ \mathrm{dense}\).
Gradient through a composition is the chain rule
(Theorem~\ref{ax:pdiv_comp}), applied at each arrow. The backward
pass walks this chain right-to-left.

\paragraph{Step 2 --- softmax cross-entropy.}
The gradient of CE w.r.t. pre-softmax logits \(z\) is the closed
form \(dz = \mathrm{softmax}(z) - t\)
(Theorem~\ref{ax:softmaxCE_grad}, proved in Ch~\ref{chap:mlp} next
to softmax itself). This is the starting upstream gradient.

\paragraph{Step 3 --- gradient w.r.t. \(b\).}
The map \(b \mapsto W x + b\) is the identity in \(b\)
(with \(W\) and \(x\) held constant): identity Jacobian
(Theorem~\ref{ax:pdiv_id}), VJP passes the gradient through
unchanged (Theorem~\ref{thm:identity_has_vjp}). So
\(\nabla_b \ell = dz\).

\paragraph{Step 4 --- gradient w.r.t. \(W\).}
The map \(W \mapsto W x + b\) has the per-entry Dense Jacobian
...
product \(\nabla_W \ell = dz \otimes x\).

\paragraph{What we just used.}
Four steps. Three theorems from this chapter (chain rule, identity
Jacobian, identity VJP); two forward-cites to Ch~\ref{chap:mlp}
(softmax CE, Dense weight Jacobian). The Ch~\ref{chap:mlp}
theorems are \emph{themselves} proved from this chapter's
foundation rules --- when you read them, you'll see
\(\pdiv\)\_finset\_sum + reindex + const + add doing the work
underneath.

That is the \emph{entire} backward pass for a working classifier.
Run \texttt{linear-sgd} and the codegen emits exactly the formulas
above as fused MLIR; the Lean proofs guarantee they're correct.

\paragraph{One more knob to taste.}
The \texttt{linear-adam} cell in the same registry trains the
identical architecture with Adam instead of SGD; comparing the two
is the smallest possible optimizer-ablation in the book and
demonstrates that the gradient computation we just derived is
optimizer-agnostic --- the same \(\nabla \ell\) values feed either
update rule. This is the Ch~\ref{chap:mlp}--\ref{chap:residual}
ablation pattern in miniature.
...
(one new operator); Ch~\ref{chap:attention} adds attention (the
biggest jump --- new matrix-level machinery plus softmax-VJP
chains). The structural rules from this chapter \emph{never
change}; later chapters add operator-specific theorems and compose
them through the chain rule we just proved.
