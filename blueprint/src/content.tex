...
\node[payoff] (payoff) at (0, -2) {Used in Chapters 3--9: every per-architecture \\ VJP unfolds through these structural rules. \\ \footnotesize (Ch~10 attention adds matrix-level machinery; see~\S\ref{sec:matrix_machinery}.)};
...
3--9 unfolds to. The full clickable dependency graph is in the
blueprint web view.
...
The map \(W_2 \mapsto W_2 h + b_2\) has the per-entry Dense Jacobian
(Theorem~\ref{ax:pdiv_dense_W}, proved in Ch~\ref{chap:mlp} from
\emph{this chapter's} foundation rules: \(\pdiv\)\_finset\_sum +
const + reindex). Chain rule (Theorem~\ref{ax:pdiv_comp}) composes
that Jacobian with the upstream gradient \(dz\) to give the outer
product \(\nabla_{W_2} \ell = dz \otimes h\).

\paragraph{Step 5 --- gradient w.r.t. \(h\) (the ReLU activation).}
Same Dense layer, now differentiated w.r.t. its input: per-entry
Jacobian by Theorem~\ref{ax:pdiv_dense} (also a Ch~\ref{chap:mlp}
theorem from this chapter's foundation rules). Chain rule again
gives \(dh = W_2^T\, dz\).
...
Symmetric to Steps 3--4. Identity VJP
(Theorem~\ref{thm:identity_has_vjp}) gives
\(\nabla_{b_1} \ell = dh_{\text{pre}}\); the Dense weight Jacobian
(Theorem~\ref{ax:pdiv_dense_W}) plus chain rule give
\(\nabla_{W_1} \ell = dh_{\text{pre}} \otimes x\).

\paragraph{What we just used.}
Eight steps. Three from this chapter (chain rule, identity Jacobian
and VJP); five foreshadowed for Ch~\ref{chap:mlp} (softmax CE, ReLU
VJP, the two Dense Jacobians, and Dense-bias-gradient is identity).
The Ch~\ref{chap:mlp} theorems are \emph{themselves} proved from
this chapter's foundation rules --- when you read them, you'll see
\(\pdiv\)\_finset\_sum + reindex + const + add doing the work
underneath.
